...
class MusicWidget:
    def __init__(self, frame, decryptFile, reloadFunc):
        ...
        self.musicLb = Label(self.txtFrame, text="BGM数", font=("", 20), width=7, borderwidth=1, relief="solid")
        self.musicLb.grid(row=0, column=0, sticky=W+E)

        self.varMusic = IntVar()
        self.varMusic.set(self.decryptFile.musicCnt)
        self.musicTextLb = Label(self.txtFrame, textvariable=self.varMusic, font=("", 20), width=7, borderwidth=1, relief="solid")
        self.musicTextLb.grid(row=0, column=1, sticky=W+E)
        self.musicBtn = Button(self.txtFrame, text="修正", font=("", 14), command=lambda:self.editVar(self.varMusic.get()))
        self.musicBtn.grid(row=0, column=2, sticky=W+E)
