...
class ScrollbarFrame():
    def __init__(self, parent, bar_x=False, bar_y=True):
        ...
        self.frame = tkinter.Frame(
            self.canvas, width=parent.winfo_width() - 8, height=parent.winfo_height())
        ...
        if bar_x:
            self.scrollbar_x = tkinter.Scrollbar(
                parent, orient=tkinter.HORIZONTAL, command=self.canvas.xview)
            self.scrollbar_x.pack(side=tkinter.BOTTOM, fill=tkinter.X)
            self.canvas.configure(xscrollcommand=self.scrollbar_x.set)

        if bar_y:
            self.scrollbar_y = tkinter.Scrollbar(
                parent, orient=tkinter.VERTICAL, command=self.canvas.yview)
